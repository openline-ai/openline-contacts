import React, {ButtonHTMLAttributes, ChangeEvent, EventHandler, FC, FormEventHandler, useRef} from 'react';
...
export const NoteEditor: FC<Props> = ({
            ...
            fetch(`/fs/file/${r.data.id}/download`)
                .then(async (response: any) => {
                    ...
                    reader.onload = function () {
                        ...
                        if (dataUrl) {
                            onTextChange(onGetFieldValue("htmlEnhanced") + `<img width="400" src='${dataUrl}' alt='${r.data.id}'>`)
                        } else {
    ...
    return (
        <div style={{display: 'flex', flexDirection: 'column',  margin: isEditMode ? '-17px -24px' : 0}}>
            ...
            <div className={` flex justify-content-end  ${isEditMode ? 'mb-3 mr-3' : 'mt-3'}`}>
                {isEditMode ? (
                    <>
                        <Button onClick={onCancel} className={`${isEditMode ? 'mb-3 mr-3' : ''}`}>
                            Cancel
                        </Button>
                        <Button onClick={onSave} mode="primary" className={`${isEditMode ? 'mb-3 mr-3' : ''}`}>
                            Save
                        </Button>
                    </>
                ) : (
